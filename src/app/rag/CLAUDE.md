...
- `chain.py` — `build_agent()` wires `create_agent` with both tools + `GuardrailMiddleware`. `search_knowledge_base(query)` retrieves across all topics; `generate_project_ideas(description, industry?, service_type?)` retrieves only from `topic=templates` (case studies). Agent infers `industry`/`service_type` from conversation context. `stream_response()` emits SSE events: `{type: "tool_call", tool, query}`, `{type: "token", token, done: false}`, `{type: "ideas", ideas}`, `{type: "done", cta, followups}`. `followups` is populated by `pick_followups()` (skipped when ideas were emitted; `[]` on picker failure).
