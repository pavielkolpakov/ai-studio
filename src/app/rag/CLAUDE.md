...
- `ideas.py` — `get_use_cases_retriever()` (Qdrant retriever filtered to `topic=use-cases`, k=6); `IdeasPayload`/`Idea` Pydantic schemas; `generate_ideas_payload(description)` runs retrieval + structured-output LLM.
