...
export function StudioLanding({ children }: { children: ReactNode }) {
  ...
  return <main className="studio-home">
    ...
    <section className="studio-hero" aria-labelledby="home-title">
      ...
      <motion.div className="hero-copy" initial={reduce ? false : { opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }}>
        ...
        <div id="scanner" className="scanner-input hero-scanner">
          <div className="scanner-label"><span className="scanner-spark" aria-hidden="true" />Your next AI opportunity starts here</div>
          {children}
        </div>
      ...
      <motion.div ref={gallery} className="hero-gallery" aria-label="Our approach" style={{ y: reduce ? 0 : drift }}>
        <motion.a href="#expertise" className="gallery-panel gallery-audit image-panel" initial={reduce ? false : { opacity: 0, y: 70, rotate: -4 }} animate={{ opacity: 1, y: 0, rotate: reduce ? 0 : -4 }} transition={{ duration: 1, delay: 0.15 }}>
        ...
        <motion.a href="#expertise" className="gallery-panel gallery-build image-panel" initial={reduce ? false : { opacity: 0, y: 90 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.28 }}>
        ...
        <motion.a href="#expertise" className="gallery-panel gallery-optimize image-panel" initial={reduce ? false : { opacity: 0, y: 70, rotate: 4 }} animate={{ opacity: 1, y: 0, rotate: reduce ? 0 : 4 }} transition={{ duration: 1, delay: 0.4 }}>
    ...
    <section id="expertise" className="studio-section expertise" aria-labelledby="expertise-title">
      ...
      <div className="expertise-grid">
        {PRICING_PAGE.paths.map((service, index) => <Reveal key={service.service} className={`service-card service-card-${index}`} delay={index * 0.07}>
          ...
          <Link to="/pricing" className="service-link"><span>{service.price}</span><span>View service <ArrowRight size={18} /></span></Link>
    ...
    <section className="studio-section studio-about" aria-labelledby="approach-title">
      ...
      <Reveal className="about-copy"><p className="studio-kicker">Designed around your business</p><h2 id="approach-title">Your product.<br />Your constraints.<br />Your advantage.</h2><p>We work with the data, workflows, and infrastructure you already have. Every technical decision starts with what your business needs.</p><Link to="/pricing" className="studio-text-link">See how we work <ArrowUpRight size={17} /></Link></Reveal>
    </section>

    <section className="studio-section studio-scanner" aria-labelledby="scanner-title">
      <Reveal><p className="studio-kicker">From a question to a clear next step</p><h2 id="scanner-title">Your next chapter.<br />Powered by possibility.</h2><p>Tell us about your product. We’ll help you find a practical place to start.</p><a href="#scanner" className="studio-button" onClick={(event) => { event.preventDefault(); document.getElementById("scanner")?.scrollIntoView({ block: "center" }); document.querySelector<HTMLTextAreaElement>("#scanner textarea")?.focus({ preventScroll: true }); }}>Explore your AI opportunity <ArrowUpRight size={17} /></a></Reveal>
    </section>
    <SiteFooter />
  </main>;
}
